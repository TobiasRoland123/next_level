...
export const AdminHeader = ({ pageList }: HeaderProps) => {
  ...
  return (
    <>
      <div className={` bg-primaryCol w-screen h-screen fixed top-0 transition-all md:hidden  z-10 overflow-scroll pb-6  ${isOpen ? "right-0" : "right-full"} flex flex-col pt-28`}>
        ...
        <div className="flex justify-center mt-10">
          <Button className="text-2xl font-bold px-8 py-6" onClick={signUserOut}>
            Log ud
          </Button>
        </div>
      </div>
      ...
      <nav className="fixed top-2 px-6 md:px-12 lg:px-20 z-10 w-full ">
        <div className={`my-3 backdrop-blur-sm ${isOpen ? "bg-primaryCol" : "bg-contrastCol/70"} flex justify-between   items-center px-4 gap-6 rounded-sm md:py-2 md:bg-contrastCol/70 max-w-screen-xl xl:mx-auto`}>
                ...
                return (
                  <>
                    {pages.page.subPages ? (
                      ...
                      <a href={pages.page.href} className="border-2 border-transparent hover:border-b-accentCol ">
                        {pages.page.pageTitle}
                      </a>
                    )}
                  </>
          ...
          <Button className="hidden md:block my-1" onClick={signUserOut}>
            Log ud
          </Button>
        </div>
      </nav>
